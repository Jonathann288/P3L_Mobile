...
class AuthClient {
  static final String url = 'reusemartshop.sikoding.id'; // pakai http dan port Laravel

  Future<Map<String, dynamic>> login(String email, String password) async {
    final uri = Uri.https('$url', '/api/auth/login'); // gunakan parse, bukan http()
  ...
  Future<MeResponse> fetchMe() async {
    ...
    final uri = Uri.https('$url', '/api/auth/me'); // sesuaikan endpoint
  ...
  Future<bool> logout() async {
    ...
    final uri = Uri.https('$url', '/api/auth/logout');
    final response = await http.post(
      uri,
      headers: {
